...
class _RemoteAlbumPageState extends ConsumerState<RemoteAlbumPage> {
  ...
  // We need an invite page that does not list all users on the server but takes an email.
  ...
  Future<void> deleteAlbum(BuildContext context) async {
    ...
    if (confirmed == true) {
      try {
        ...
        context.pushRoute(const AlbumsRoute());
      } catch (e) {
        ImmichToast.show(
          context: context,
  ...
  // TODO: Review this for commenting and liking (I think)
  // Future<void> showActivity(BuildContext context) async {
  //   context.pushRoute(const DriftActivitiesRoute());
  // }

  // TODO: Rethink user experience, the bottom sheet contains a lot of options
  // and we might not even use the grid timeline here either
